...
addwindowButton.addEventListener('click', () => {
  ...
  windowPositionInput.className = 'form-select';
  windowPositionInput.placeholder = ' Vanaf links (mm)';
...
function tekenmuur() {
  ...
  document.getElementById('hoogte-muur').innerHTML = (muurDy / 1000) + '(M)';
...
if (windowWidth < muurDx && (windowHeight < muurDy)) {
    ...
    const windowbreedte = Math.floor( windowWidth / SteenMetVoeg ) ; // raam breedte delen op de steenmaat met voeg, zo weten we hoeveel stenen breed de deur is
    ...
    ctx.clearRect(windowPosition - voegDx + 5, SteenMetVoegHoogte + 5, nieuwewindowbreedte + voegDx, nieuwewindowhoogte); // de +5 is voor de canvas
    ctx.drawImage(raam, windowPosition+5, SteenMetVoegHoogte + 5, nieuwewindowbreedte - voegDx, nieuwewindowhoogte - voegDx);
  }
  else {
